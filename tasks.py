...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_REMOTE_JOBS_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_remote_jobs.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_remote_jobs.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_remote_jobs.nautobot_ver,
        "PYTHON_VER": context.nautobot_remote_jobs.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_remote_jobs.project_name}",
        f'--project-directory "{context.nautobot_remote_jobs.compose_dir}"',
    ]

    for compose_file in context.nautobot_remote_jobs.compose_files:
        compose_file_path = os.path.join(context.nautobot_remote_jobs.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_remote_jobs.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_remote_jobs.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.nautobot_remote_jobs.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_remote_jobs.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_remote_jobs.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_remote_jobs.python_ver
    dockerfile_path = os.path.join(context.nautobot_remote_jobs.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_remote_jobs.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_remote_jobs"
...
@task(
    help={
        "version": "Version of Remote Jobs to generate the release notes for.",
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_remote_jobs"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_remote_jobs") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_remote_jobs.migrations"
        )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_remote_jobs/templates/"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_remote_jobs.local):
